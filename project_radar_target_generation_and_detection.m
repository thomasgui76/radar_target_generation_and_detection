...
V = -20;     % initial velocity of target
Light_Speed = 3e8;
Range_Resolution = 1;
Max_Range = 200;
...
Band_Width = Light_Speed/(2*Range_Resolution);
Tchirp = 5.5*2*Max_Range/Light_Speed;
Slope = Band_Width/Tchirp;
% disp(Slope);
...
for i=1:length(t)         
    ...
    % *%TODO* :
    %Now by mixing the Tran)smit and Receive generate the beat signal
...
%normalize.
sig_fft1 = fft(Mix,[],1);        % FFT along with Nr or row.
